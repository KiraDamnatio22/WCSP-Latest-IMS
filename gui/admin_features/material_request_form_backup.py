...
class MaterialReqForm(ctk.CTkToplevel):
    ...
    def setup_installation_checklist(self):
        ...
        # Headers
        self.setup_headers(self.indoor_frame)
        self.setup_headers(self.outdoor_frame)
        self.setup_headers(self.supply_frame)
        self.setup_headers(self.drainage_frame)
        ...
        self.copper_qty_frames = self._create_qty_frame(indoor_items_frame, 645)
        self.insulation_qty_frames = self._create_qty_frame(indoor_items_frame, 655)

        # Render items
        self._render_items(indoor_items_frame, indoor_items, is_indoor=True)
        self._render_items(outdoor_items_frame, outdoor_items, is_indoor=False)
...
# app = ctk.CTk()
# app.geometry("500x250+780+380")
# app.resizable(False, False)
# app.configure(fg_color="WHITE")


# def open_request_form():
#     MaterialReqForm(app, fg_color="#F0F0F0")

# ctk.CTkLabel(app, text="Request", font=("Roboto", 20)).pack()

# request_frame = ctk.CTkFrame(app, fg_color="white", border_width=1)
# request_frame.pack(pady=(50, 0))

# request_btn = ctk.CTkButton(request_frame, text="Open Request Form", height=30, width=30, fg_color=COLOR_GREEN_HOVER, hover_color=COLOR_GREEN, command=open_request_form, anchor="e")
# request_btn.grid(row=0, column=0, padx=20, pady=15)
